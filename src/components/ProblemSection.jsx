...
const HiddenFrictionVisual = ({ scrollYProgress }) => {
  ...
  return (
    <div className="relative w-full h-[300px] sm:h-[350px] flex items-center justify-between py-6 rounded-3xl border overflow-hidden px-4 sm:px-10" style={{ background: 'rgba(255,255,255,0.03)', borderColor: 'rgba(255,215,0,0.08)' }}>
      ...
      >
        ...
        <div className="absolute left-10 right-10 top-1/2 -translate-y-1/2 h-[2px] bg-white/5 z-0 rounded-full" />
          ...
          >
            ...
            <motion.div
              animate={{
                ...
                borderColor: ["rgba(255,215,0,0.1)", "rgba(255,215,0,0.1)", "rgba(255,100,100,0.4)", "rgba(255,215,0,0.1)"],
                boxShadow: [
                  "0px 4px 6px rgba(0,0,0,0.2)", 
                  "0px 4px 6px rgba(0,0,0,0.2)", 
                  "0px 8px 15px rgba(239,68,68,0.2)", 
                  "0px 4px 6px rgba(0,0,0,0.2)"
                ]
              }}
              transition={{
              ...
              className="w-full aspect-[4/3] flex items-center justify-center rounded-lg border shadow-sm relative cursor-default transition-colors duration-300" style={{ background: 'rgba(255,255,255,0.05)' }}
            >
              <span className="text-[10px] sm:text-xs font-semibold text-white/70 whitespace-nowrap">{block.label}</span>
              
              {/* HOVER INTERACTION: Expansion Info Box */}
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[130px] text-white text-[9px] sm:text-[10px] py-2 px-3 rounded-md opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 pointer-events-none z-50 text-center shadow-lg" style={{ background: 'rgba(255,215,0,0.15)', border: '1px solid rgba(255,215,0,0.2)' }}>
                <span className="leading-snug">{block.info}</span>
                <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 border-x-[5px] border-x-transparent border-b-[6px]" style={{ borderBottomColor: 'rgba(255,215,0,0.15)' }}></div>
              </div>
            </motion.div>
          </motion.div>
        ))}
...
const ProblemSection = () => {
  ...
  return (
    <section 
      ref={containerRef} 
      className="relative rounded-t-[2rem] md:rounded-t-[3rem] py-16 md:py-24 px-4 sm:px-6 md:px-20 overflow-hidden w-full max-w-[100vw]" style={{ background: '#0B1A33', boxShadow: '0 -4px 30px rgba(0,0,0,0.3)' }}
    >

      {/* Subtle Tension Background Grid */}
      <div className="absolute inset-0 opacity-[0.04] pointer-events-none"
        style={{
          backgroundImage: "repeating-linear-gradient(45deg, rgba(255,215,0,0.3) 0, rgba(255,215,0,0.3) 1px, transparent 0, transparent 50%)",
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
        ...
        <div className="w-full lg:w-1/2 flex flex-col text-center lg:text-left">
          ...
          <motion.p
            ...
            className="text-base md:text-xl text-white/45 mb-10 max-w-lg mx-auto lg:mx-0 font-medium relative z-10"
          >
          ...
          >
            {problems.map((text, i) => (
              ...
              >
                <div className="mt-[6px] flex-shrink-0 w-4 h-4 rounded-full border-2 border-[#FFD700]/40 flex items-center justify-center">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#FFD700]/60" />
                </div>
                <span className="text-white/65 font-medium text-base md:text-lg">{text}</span>
              </motion.li>
            ))}
          </motion.ul>
        </div>
