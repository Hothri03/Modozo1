  ...
  <div className="relative w-full h-12 flex items-center justify-center overflow-hidden">
    {[0, 1, 2].map(i => (
      <motion.div
        ...
        className="absolute w-6 h-8 bg-black/5 border border-yellow-400/30 rounded-md flex items-center justify-center"
      >
        <div className="w-3 h-0.5 bg-black/20 rounded-full" />
      </motion.div>
    ))}
    <motion.div
    ...
    {[0, 1, 2].map(i => (
      <div key={i} className="relative">
        <motion.div 
          ...
          className="w-8 h-8 border border-black/10 rounded-lg"
        />
        <motion.div
...
const ImpactSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="flex flex-col justify-center py-16 md:py-24 px-4 sm:px-6 bg-yellow-100 text-text-primary rounded-t-[2rem] md:rounded-t-[3.5rem] relative w-full overflow-hidden max-w-[100vw]"
    >
      ...
      <div className="max-w-[1400px] mx-auto relative z-10 w-full flex flex-col">
        ...
        <div className="relative w-full flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-16 px-2">
          ...
          {/* Status Badge */}
          <div className="mt-8 md:mt-0 flex items-center justify-center gap-3 bg-white/60 px-5 py-2.5 rounded-full border border-yellow-400/30 backdrop-blur-md shadow-sm self-start md:self-end">
            <span className={`text-xs font-bold uppercase tracking-widest transition-colors duration-1000 ${phase === 'chaos' ? 'text-gray-600' : 'text-gray-300'}`}>
              Before
            </span>
            
            {/* Pill Indicator */}
            <div className="w-12 h-5 bg-black/5 rounded-full p-0.5 relative flex items-center">
              <motion.div 
            ...
            <span className={`text-xs font-bold uppercase tracking-widest transition-colors duration-1000 ${phase === 'ordered' ? 'text-yellow-600' : 'text-gray-300'}`}>
              With Modozo
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 items-stretch justify-center gap-4 lg:gap-6 relative z-10 w-full">
            {impacts.map((item, i) => (
              ...
              >
                ...
                <div className="mb-2">
                  <span className={`text-4xl lg:text-5xl font-black tracking-tighter transition-colors duration-1000 ${phase === 'ordered' ? 'text-yellow-500 drop-shadow-sm' : 'text-gray-400'}`}>
                    <AnimatedCounter value={item.value} delay={i * 0.15} active={phase === 'ordered'} />
                  </span>
                  <span className={`text-xl lg:text-2xl font-bold ml-1 transition-colors duration-1000 ${phase === 'ordered' ? 'text-text-primary/90' : 'text-gray-400'}`}>{item.suffix}</span>
                </div>
                
                <p className={`text-xs lg:text-sm font-bold uppercase tracking-widest transition-colors duration-1000 ${phase === 'ordered' ? 'text-text-primary/80' : 'text-gray-400'}`}>
                  {item.title}
                </p>
